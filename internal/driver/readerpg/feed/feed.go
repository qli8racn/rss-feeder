...
func (r *repository) Save(ctx context.Context, feed domain.Feed, userID int64) (int64, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		RETURNING id
	`, userID, feed.FeedURL, feed.Title, time.Now()).Scan(&id)
	if err != nil {
		return 0, err
	}
	return id, nil
...
func (r *repository) FindByURL(ctx context.Context, url string, userID int64) (*domain.Feed, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		FROM feeds WHERE feed_url = $1 AND user_id = $2
	`, url, userID)
...
func (r *repository) Register(ctx context.Context, url string, userID int64) error {
	res, err := r.db.ExecContext(ctx,
		`INSERT INTO feeds (user_id, feed_url) VALUES ($1, $2) ON CONFLICT (user_id, feed_url) DO NOTHING`,
		userID, url)
	if err != nil {
		return err
	}
...
// 所有権チェックを articles の削除より前に行うことで、他ユーザーの記事も守る）。
func (r *repository) Remove(ctx context.Context, id int64, userID int64) error {
	...
	var exists int64
	err = tx.QueryRowContext(ctx, `SELECT id FROM feeds WHERE id = $1 AND user_id = $2`, id, userID).Scan(&exists)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM feeds WHERE id = $1 AND user_id = $2`, id, userID); err != nil {
		return err
	}
	return tx.Commit()
}
