...
func (r *repository) FindByName(ctx context.Context, name string) (*domain.User, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, name, created_at FROM users WHERE name = $1`, name)
...
func (r *repository) Create(ctx context.Context, name string) (*domain.User, error) {
	row := r.db.QueryRowContext(ctx, `INSERT INTO users (name) VALUES ($1) RETURNING id, created_at`, name)
	var u domain.User
	u.Name = name
	var createdAt sql.NullTime
